...
const get_all_categories = async (chatId, page = 1, message_id = null) => {
  ...
  clear_draft_product()  //statusi  nolga teng bo'lgan mahsulotni o'chiradi
...
const show_category = async (chatId, id, page = 1) => {
  ...
  let limit = 5;
  let skip = (page - 1) * limit;
  let products = await Product.find({ category: category._id, status: 1 }).skip(skip).limit(limit).sort({ _id: -1 }).lean()
  ...
  bot.sendMessage(chatId, `${category.title} turkumidagi kategoriyalar mahsulotlar ro'yxati`, {
    reply_markup: {
      remove_keyboard: true,
      inline_keyboard: [
        ...list,
...
const remove_category = async (chatId, id) => {
  ...
  if (user.action !== "del_category") {
    ...
    bot.sendMessage(chatId, `Siz ${category.title} turkumni o'chirmoqchisiz qaroringiz qa'tiymi`, {
      reply_markup: {
        inline_keyboard: [
          [
            {
    ...
    bot.sendMessage(chatId, `${category.title} turkumi o'chirildi Menyudan tanlang`)
  }
